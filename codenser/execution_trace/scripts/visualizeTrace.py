...
matrix = pd.read_csv('execution_trace/data/graph_matrix.csv')

methods = matrix[matrix.columns[0]]
...
for i in range(0, methods.size):
    inWeight[methods[i]] = 0

for i in range(0,methods.size):
    callingMethod = methods[i]
    for j in range(0, methods.size):
        calledMethod = methods[j]
        weight = matrix.loc[j,callingMethod]
        if weight != 0:
            G.add_edge(callingMethod,calledMethod, weight=weight)
            inWeight[calledMethod] = inWeight[calledMethod] + weight
...
for node in G.nodes():
    ...
    node_text.append("Method: " + node + "\n Here is where the summarization goes")
...
# # nx.draw_shell(G,with_labels=True, node_color=range(methods.size), cmap=plt.cm.Blues)
# # plt.show()
